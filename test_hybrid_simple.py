"""
Простой benchmark тест для /search_hybrid эндпойнта
Проверяет качество гибридного поиска (RRF: vector + tags)
"""
import json
import requests
...
# Настройки API
API_URL = "http://localhost:8000/search_hybrid"
...
def test_single_query(question: str, target_table: str, top_n: int = 50) -> Dict:
    ...
    payload = {
        "query": question,
        "top_n": top_n,
        "vector_weight": 1.0,  # Только vector search
        "tags_weight": 0.0,    # Отключить tags
        "method": "dot_product",
        "rrf_k": 60
    }
...
def run_all_benchmarks(top_n: int = 50) -> Dict:
    ...
    print(f"Запуск {len(benchmarks)} тестов для /search_hybrid...")
    print("Параметры: vector_weight=1.0, tags_weight=0.0, method=dot_product (ТОЛЬКО VECTOR)")
    print("=" * 80)
    ...
    for idx, benchmark in enumerate(benchmarks, start=1):
        ...
        if result["position"] is None:
            ...
            print(
                ...
                f"(rrf: {result['rrf_score']:.4f}, vec: #{result['vector_rank']}, tags: #{result['tags_rank']})"
            )
        else:
            print(
