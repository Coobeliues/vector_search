"""
Benchmark тест для проверки качества ранжирования /search_rerank эндпойнта
Проверяет, попадает ли целевая таблица (target_table) в топ результатов
"""
import json
import requests
...
# Настройки API
API_URL = "http://localhost:8000/search_rerank"
...
def test_single_query(question: str, target_table: str, top_n: int = 50) -> Dict:
    """
    Отправить запрос к API и проверить позицию целевой таблицы
...
def run_all_benchmarks(top_n: int = 50) -> Dict:
    ...
    for idx, benchmark in enumerate(benchmarks, start=1):
        ...
        if result["position"] is None:
            ...
            print(f"  ✅ TOP_1 - '{target_table}' на 1 месте! (score: {result['score']:.3f})")
        else:
            print(f"  ⚠️  RANK_{result['position']} - '{target_table}' на {result['position']} месте (score: {result['score']:.3f})")
...
def show_failed_tests(results: List[Dict], threshold_position: int = 5):
    ...
    for idx, result in enumerate(failed, start=1):
        ...
        if result['position'] is None:
            ...
            print(f"    ⚠️  Статус: позиция {result['position']} (score: {result['score']:.3f})")
        print(f"    Топ-3 результата: {', '.join(result['top_3_tables'])}")


if __name__ == "__main__":
    # Запустить все тесты
    benchmark_results = run_all_benchmarks(top_n=100)
    ...
    output_file = "/home/keosido/Desktop/Vector_search/benchmark_results_dot_product.json"
